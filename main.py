...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _on_analytics_row_selected(self, table):
        ...
        # Switch to Saved Documents tab (1)
        self.tabs.setCurrentIndex(1)
        target_index = str(doc.get('ExcelIndex', '')).strip()

        def select_row():
            table_saved = self.table_saved
            if not target_index:
                return
            # Try to match only by ExcelIndex
            for i in range(table_saved.rowCount()):
                idx = table_saved.item(i, 1).text().strip() if table_saved.item(i, 1) else ''
                if idx == target_index:
                    table_saved.selectRow(i)
                    self.on_saved_double_click(i, 0)
                    break

        # Give UI time to update, then select
        QtCore.QTimer.singleShot(150, select_row)
